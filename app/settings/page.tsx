...
function SettingsContent() {
  ...
  async function analyzeFromWebsite() {
    ...
    try {
      ...
      setAnalyzeMsg('Brand voice filled in! Review the fields below and save.')
    } catch (e) {
          ...
          <Card className="border-violet-100 bg-violet-50/40">
            <CardHeader>
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-base">Generate from existing content <span className="text-zinc-400 font-normal text-sm">(optional)</span></CardTitle>
              </div>
              <CardDescription>Paste captions, an Instagram bio, or any brand copy — Claude will extract your voice and fill in the fields below.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              {importMode === 'text' ? (
                <div className="space-y-2">
                  ...
                  >
                    {analyzing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                    {analyzing ? 'Analyzing…' : 'Generate brand voice'}
                  </button>
                </div>
              ) : (
